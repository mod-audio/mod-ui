...
function InstallationQueue() {
    ...
    this.installUsingURI = function (uri, usingLabs, callback) {
        ...
        if (usingLabs === 'auto') {
            ...
            // regular request from plugin store, so we know where the plugin is supposed to come from
            $.ajax({
    ...
    this.installNext = function () {
        ...
        var args = {
            headers: {
                'Authorization' : 'MOD ' + desktop[tokenProp],
            }
        }
        var trans = new SimpleTransference(bundle.file, '/effect/install', { from_args: args })

        trans.reauthorizeDownload = bundle.usingLabs ? desktop.authenticateDeviceLabs : desktop.authenticateDevice;
