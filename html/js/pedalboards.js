...
JqueryClass('pedalboardBox', {
	...
	options = $.extend({
	    ...
	    remove: function(pedalboard, callback) { callback() },
	    load: function(pedalboardId, callback) { callback() },
	    duplicate: function(pedalboard, callback) { callback(pedalboard) }
	}, options)
	...
	rendered.find('.js-duplicate').click(function() {
		...
		dupRendered.animate({ opacity: 1 }, 200)
	    })
	    return false
	})
...
JqueryClass('bankBox', {
	...
	var searcher = new PedalboardSearcher($.extend({
 	...
 	options.pedalboardCanvas.sortable({ 
		...
		ui.item.find('.js-remove').show().click(function() {
		    ui.item.animate({ opacity: 0, height: 0 }, function() { ui.item.remove() })
		    self.bankBox('save')
		})
	...
	self.data('bankCanvas').children().each(function() {
	    var bank = $(this)
	    var pedalboards = ( bank.data('selected')
				? self.data('pedalboardCanvas')
				: bank.data('pedalboards')
			      )
	    var pedalboardData = []
    ...
    create: function() {
	var self = $(this)
	bank = self.bankBox('renderBank', { 'title': '',
					    'pedalboards': []
					  })
	...
	rendered.find('.js-remove').click(function() {
	    rendered.animate({ opacity: 0, height: 0 }, function() { rendered.remove() })
	    self.bankBox('save')
	})
