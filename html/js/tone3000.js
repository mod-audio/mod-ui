...
// one; the real device, on plain http:// over a LAN IP, is not. Keep a small
// synchronous SHA-256 implementation for the latter case; PKCE requires S256,
// so silently weakening to `plain` is not acceptable.
function tone3000Sha256Fallback (input) {
    ...
    var constants = [0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    ...
    for (var offset = 0; offset < bytes.length; offset += 64) {
        ...
        for (i = 0; i < 64; i++) {
            ...
            var choice = (e & f) ^ ((~e) & g)
            var temp1 = (h + sigma1 + choice + constants[i] + words[i]) | 0
    ...
    for (i = 0; i < hash.length; i++) {
        ...
        result[(i * 4) + 1] = hash[i] >>> 16
        result[(i * 4) + 2] = hash[i] >>> 8
        result[(i * 4) + 3] = hash[i]
    }
    return result
}
...
function tone3000Sha256Base64Url (input) {
    ...
    return Promise.resolve(tone3000Base64Url(tone3000Sha256Fallback(input)))
}
